...
const MainLand = (props) => {
  ...
  const [sizeMapHorizontal, setSizeMapHorizontal] = useState(200);
  ...
  const loadMap = async () => {
    try {
      ...
      if (addressMapLoad == Config.LAND_SUT_ADDR) {
        /** SUT map */
        setEnterVelaverseLink("https://multiplay.velaverse.io/sut");
      } else {
        /** Yaamo map default */
      ...
      mapDetected.map((ele, index) => {
        if (!newMap[sizeMapHorizontal - 1 - ele.y])
          newMap[sizeMapHorizontal - 1 - ele.y] = [];
        newMap[sizeMapHorizontal - 1 - ele.y][ele.x] = ele;
      });
      ...
      allLands.map((land, index) => {
        ...
        newMap[sizeMapHorizontal - 1 - land.y][land.x].tokenId = land.token_id;
        newMap[sizeMapHorizontal - 1 - land.y][land.x].owner = land.own_addr;
      });
  ...
  const onBuyLand = async () => {
    ...
    if (bought && bought.status) {
      const transfers = bought.events.Transfer;
    ...
    if (mapData.length > 0) {
      ...
      for await (const [indexA, valA] of mapData.entries()) {
        ...
        for (let indexB = 0; indexB < valueA.length; indexB++) {
          ...
          if (typeof _cords.owner !== "undefined" && _cords.prefabId == 0) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 justify-items-stretch px-4 py-3">
          <div className="flex flex-col label-land-statuses py-2">
            ...
            <div className="flex space-x-4">
              ...
              <div className="flex items-center">
                <div className="inline-flex w-4 h-4 bg-gray-500">&nbsp;</div>
          ...
          >
            <div>
              ...
              <div className="mt-1 text-xs">
                ...
                <div className="mb-1">
                  <label className="mb-1 block">Place your Image URL:</label>
                  <input
                    type="url"
                    className="block w-full border border-gray-300 rounded-md shadow-sm py-1 px-1 text-xs"
                ...
                <div className="mb-1">
                  <label className="mb-1 block">Place your external URL:</label>
                  <input
                    type="url"
                    className="block w-full border border-gray-300 rounded-md shadow-sm py-1 px-1 text-xs"
