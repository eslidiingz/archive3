...
import { getCreatorById } from "utils/models/Creator";
import Link from "next/link";

const ManagecreatorDetail = () => {
  ...
  useEffect(() => {
    ...
    if (router.isReady && mounted) initialize();
  ...
  const initialize = async () => {
    try {
      const responseCreator = await handleFetchCreatorById();
      console.log(responseCreator);
      setCreator(responseCreator);
    } catch (err) {
      console.error(err.message);
    }
  };

  const handleFetchCreatorById = async () => {
    try {
      const creator = await getCreatorById(router.query?.whitelistId);
      return creator || {};
    } catch {
      return {};
    }
      ...
      <div className="container">
        <div className="row">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              {/* Search */}
              <div className="flex items-center max-w-md mx-autos bg-whites rounded-lg " x-data="{ search: '' }">
                ...
                <div>
                  <button type="submit" className="flex items-center bg-gray justify-center w-12 h-12 text-white rounded-r-lg" disabled="search.length == 0">
            ...
            <div className="layout10" Align="left">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2 layout12">
                  <div>
                    <img src="/assets/images/profile/Ellipse 48-6.png" className="img-radius img-ma" width="45px" />
                  ...
                  <div>
                    ...
                    {creator?.register?.portfolio?.map?.((portfolio, index) => (
                      <p key={`${portfolio.value}_${index}`}>
                        <Link href={portfolio.value}>
                          <a className="text16" target="_blank">
                            {portfolio.value}
                          </a>
                        </Link>
                      </p>
